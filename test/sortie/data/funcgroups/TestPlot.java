...
public class TestPlot extends ModelTestCase {
  ...
  public void testReadV6ParFile() {
    ...
    try {
      ...
      assertEquals(1, oPlot.getNumberOfTimesteps());
  ...
  public void testcreateBehaviorFromParameterFileTag() {
    ...
    try {
      oManager = new GUIManager(null);
      oManager.clearCurrentData();
      sFileName = writeNeutralFile1();
      oManager.inputXMLParameterFile(sFileName);
      BehaviorTypeBase[] p_oObjs = oManager.getAllObjects();
      PlotBehaviors oPlot = null;
      for (int i = 0; i < p_oObjs.length; i++) {
        if (p_oObjs[i] instanceof PlotBehaviors)
          oPlot = (PlotBehaviors) p_oObjs[i];
      }
      
      assertTrue(oPlot != null);
      assertEquals(1, oPlot.getAllInstantiatedBehaviors().size());
      oPlot.createBehaviorFromParameterFileTag("plot");
      assertEquals(1, oPlot.getAllInstantiatedBehaviors().size());
      System.out.println("testcreateBehaviorFromParameterFileTag succeeded for TestPlot.");
    }
    catch (ModelException oErr) {
      fail("Caught error with message " + oErr.getMessage());
    }
   ...
   */
  private String writeNeutralFile1() throws IOException {
